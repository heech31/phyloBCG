...
setwd("/general/home/hcchung/graphical/p50cdf/m3/")
...
Rep50 <- foreach( irep = 1:nrep)%dopar%{
    ...
    # Get estimated graph with threshold 0.5
    ...
    gibbsSummary <- list(
                        What = What, # What matrix corresponds adjacency matrix E
                        Wtrue = Wtrue,
                        edgeHat = edgeHat,
                        edgeTrue = edgeTrue,
                        edgeTotal = edgeTotal,
...
save.image(paste(resultPath,"Gibbs_cdf_T_",iseed,".RData",sep=""))
