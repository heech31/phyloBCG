...
setwd("/general/home/hcchung/graphical/p50cdf/m1/")
...
source(paste(funcPath,"orclGibbs.R",sep=""))
...
# The number of true connected edges
nconnect <- sum( as.matrix(Wtrue)[ upper.tri(Wtrue) ] )
pijk <- nconnect / choose(p,2) # True graph density
...
Rep50 <- foreach( irep = 1:nrep)%dopar%{
    ...
    gibbsSample <- orclGibbs(x, delta_mc, zhat_mc, R_mc, v0_mc, h, tau_mc, pijk,
...
save.image(paste(resultPath,"Gibbs_cdf_O_",iseed,".RData",sep=""))
