...
for (iter in 1:(burnin + nmc)) {
  if (iter %% 2000 == 0) {
	...
	for( jj in 1:p ){
		...
		# Uold is K x p 
		uujj <- as.vector( crossprod(Uold[,-jj], Uold[,jj]) ) # T_{-j}'*tj in the manucript
    ...
    # a and b are the lower and upper bound, respectively
	  ...
	  full_ind_noj <- ind_noj
		...
		uv <- Uold[,-jj] # the latent position of the jth variable
	  ...
	  # Full conditional covariance matrix
		Delj   <- solve( tcrossprod(Uold[,-jj],Uold[,-jj]) + diag(1/Phij,2) ) # Two components of tj are independent fo Phi_j = diag(1/Phij,2)
