...
## Load the SPIEC-EASI result
load(paste(resultPath,"result.RMPspieceasi.RData",sep="") )
##################################################################################
##################################################################################
##################################################################################
## se is fit.spieceasi (SPIEC-EASI result)
# StARS-selected lambda index based on the threshold (default = 0.01)
opt.S <- se$select$stars$opt.index
# Estimated adjacency matrix from sparse graphical modeling technique ("mb" method) (1 = edge, 0 = no edge)
adj.S <- as.matrix(se$est$path[[opt.S]])
# Estimated partial correlation coefficient, same as negative precision matrix.
pcor.S <- as.matrix(SpiecEasi::symBeta(se$est$beta[[opt.S]], mode = 'maxabs'))

colnames(adj.S) <- colnames(x)
rownames(adj.S) <- colnames(x)  
grph <- graph_from_adjacency_matrix(adj.S, mode="undirected")
...
pdf(paste(figPath,"RMPnetwork.se_1e-3.pdf",sep=""),width=12.5,height=5)
par(oma=c(1,1,1,1),mar=c(0,0,1,1))
plot(grph, vertex.label=colnames(adj.S), vetex.label.color="black", vertex.color = membership(ceb)+2, 
     asp=0.4, edge.width=0.3, layout = layout_with_kk , vertex.size=rep(3,54), main="SPIEC-EASI" ) 
dev.off()
...
tile.om.hat <- tile.om.hat + annotate("text", x = (p+2)/2, y = -0.7, angle = 0, label = "SPIED-EASI Partial Correlation")
...
ggsave(paste(figPath,"parCor.seRMP.pdf",sep=""),tile.om.hat, width=12,height=9)
save(pcor.S, file = paste(resultPath,"pcor.seRMP.RData",sep=""))
