...
PDM <- function(x, z, R, delta, nK = 4, nL = 11 ){
  ...
  for( ii in 1:n){
    ...
    pivot.unif <- rep(0, length(z.obs) )
    for( io in 1:length(z.obs)){
      pivot.unif[io] <- 
        ptmvnorm.marginal(z.obs[io], n=io, mean=mu.io, sigma=Delta.io, lower=lower)
    }

    pivots[ii, i.obs.flag] <- pivot.unif
  ...
  for( jj in 1:p){ #plot( pivots[!is.na(pivots)])#jj=1
